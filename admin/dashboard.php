...
<body class="bg-gray-50 flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-8">
            <h2 class="text-3xl font-black text-gray-800 mb-6">Dashboard Overview</h2>
            
            <!-- Statistics Dashboard -->
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div class="bg-white p-6 rounded-2xl shadow-sm border-2 border-transparent hover:border-blue-100 transition relative overflow-hidden group">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border-2 border-transparent hover:border-green-100 transition relative overflow-hidden">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border-2 border-transparent hover:border-yellow-100 transition relative overflow-hidden">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border-2 border-transparent hover:border-purple-100 transition relative overflow-hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="col-span-1 space-y-6">
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <h2 class="font-black text-lg text-gray-800">Security</h2>
                        </div>
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <div class="flex items-center gap-3 mb-4">
                            <h2 class="font-black text-lg text-gray-800">Billing Settings</h2>
                        </div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Default Visit Fee (Rs.)</label>
                        <input type="number" id="visitFeeInput" value="<?= htmlspecialchars($currentFee) ?>" class="w-full border-2 border-gray-200 p-3 mb-3 rounded-xl font-bold focus:border-teal-500">
                ...
                <!-- Database Overview -->
                <div class="col-span-1 md:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-[600px]">
                    <h2 class="text-xl font-black text-gray-800 mb-4">Quick Search & Edit</h2>
                    <input type="text" id="adminSearchInput" class="w-full border-2 border-gray-200 p-4 rounded-xl mb-4 font-bold focus:border-teal-500 focus:ring-0" placeholder="Search by name or phone...">
                    
                    <div class="flex-1 overflow-y-auto pr-2">
                        <table class="w-full text-left border-collapse">
                            <thead class="sticky top-0 bg-white shadow-sm z-10">
                                <tr class="border-b-2 border-gray-100 text-gray-500 text-sm uppercase tracking-wide">
                                    <th class="p-3 font-bold">ID</th>
                                    <th class="p-3 font-bold">Name</th>
                                    <th class="p-3 font-bold">Phone</th>
                                    <th class="p-3 font-bold hidden md:table-cell">Gender</th>
        ...
        document.getElementById('adminSearchInput').addEventListener('input', async (e) => {
            ...
            tbody.innerHTML = patients.map(p => `
                <tr class="border-b border-gray-50 hover:bg-gray-50 transition group">
                    <td class="p-3 text-gray-400 font-bold text-xs">#${p.PatientID}</td>
                    <td class="p-3 font-bold text-gray-800">${p.FirstName} ${p.LastName}</td>
                    <td class="p-3 font-mono text-sm">${p.Phone}</td>
                    <td class="p-3 hidden md:table-cell text-sm">${p.Gender}</td>
                    <td class="p-3 text-right">
                        <div class="flex justify-end gap-2 outline-none">
                            <a href="edit_patient.php?id=${p.PatientID}" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg text-sm hover:bg-gray-800 hover:text-white font-bold transition">Edit</a>
                            <a href="../doctor/history.php?patient_id=${p.PatientID}" target="_blank" class="px-4 py-2 bg-blue-50 text-blue-600 rounded-lg text-sm hover:bg-blue-600 hover:text-white font-bold transition">History</a>
                        </div>
                    </td>
                </tr>
            `).join('');
